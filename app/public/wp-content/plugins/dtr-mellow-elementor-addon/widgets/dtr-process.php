	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'placeholder' 	=> esc_html__( 'Number', 'mellow' ),
				'label_block'	=> true,
				'show_label'	=> true,
				'separator' 	=> 'before',
			]
		...
		$this->add_control(
			'heading_m_bottom',
			[
				'label' => esc_html__( 'Margin Bottom to Heading', 'mellow' ),
				...
				'separator' 	=> 'before',
				'selectors' => [
					'{{WRAPPER}} .dtr-process__heading' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_responsive_control(
			'box_padding_right',
			[
				'label' => esc_html__( 'Padding Right to Process Item', 'mellow' ),
				'type' => Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'max' => 200,
				...
				'separator'		=> 'before',
				'selectors' => [
					'{{WRAPPER}} .dtr-process' => 'padding-right: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' 	=> '',
				'selectors'	=> [
					'{{WRAPPER}} .dtr-process::before'	=> 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			'dot_color',
			[
				'label' 	=> esc_html__( 'Dot / Rings Color', 'mellow' ),
				'type' 		=> Controls_Manager::COLOR,
				'default' 	=> '',
				'selectors'	=> [
					'{{WRAPPER}} .dtr-ripple-dot'	=> 'background-color: {{VALUE}};',
					'{{WRAPPER}} .dtr-ripple-ring'	=> 'border-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' 	=> '',
				'selectors'	=> [
					'{{WRAPPER}} .dtr-process__content'	=> 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
            ...
            [
                'name' 		=> 'content_typo',
				'label' 	=> esc_html__( 'Typography', 'mellow' ),
                'selector' 	=> '{{WRAPPER}} .dtr-process__content',
            ]
        );
	...
	protected function render() {
        ...
        <div class="dtr-process">			
			...
			<?php if ( ! empty( $settings['heading'] ) ) { ?>
				<<?php echo esc_attr( $settings['heading_size'] ) ?> class="dtr-process__heading">					
				<?php if ( ! empty( $settings['number'] ) ) { ?>
				<span class="dtr-process__number"><?php echo esc_html( $settings['number'] ); ?></span>
				<?php } ?>
				<?php echo wp_kses_post( $settings['heading'] ); ?></<?php echo esc_attr( $settings['heading_size'] ) ?>>
			<?php } ?>
			<?php if ( ! empty( $settings['process_content'] ) ) { ?>
					<div class="dtr-process__content"><?php echo wp_kses_post( $settings['process_content'] ) ?></div>
			<?php } ?>  
...
<?php	}
	...
	protected function content_template() {
        ...
        <div class="dtr-process">	
			...
			<# if ( settings.heading != '' ) { #>					
				<{{ settings.heading_size }} class="dtr-process__heading"><# if ( settings.number != '' ) { #>
					<span class="dtr-process__number">{{{ settings.number }}}</span>
				<# } #>{{{ settings.heading }}}</{{ settings.heading_size }}>
			<# } #>  					
			<# if ( settings.process_content != '' ) { #>
				<div class="dtr-process__content">{{{ settings.process_content }}}</div>
			<# } #>    
